...
exports.rfc1123 = exports.rewriteSetCookies = exports.rewriteProxyResponseHeaders = exports.rewriteProxyRequestHeaders = exports.rewriteDomain = exports.parseHost = exports.cookieAsString = exports.X_PROXY_REQUEST_URL = exports.X_MOBIFY_REQUEST_CLASS = exports.X_HEADERS_TO_REMOVE = exports.X_HEADERS_TO_ADD = exports.MAX_URL_LENGTH_BYTES = exports.Headers = exports.ALLOWED_CACHING_PROXY_REQUEST_HEADERS = void 0;
var _setCookieParser = require("set-cookie-parser");
var _headerCase = _interopRequireDefault(require("header-case"));
...
const X_PROXY_REQUEST_URL = exports.X_PROXY_REQUEST_URL = 'x-proxy-request-url';
const X_MOBIFY_REQUEST_CLASS = exports.X_MOBIFY_REQUEST_CLASS = 'x-mobify-request-class';
const MAX_URL_LENGTH_BYTES = exports.MAX_URL_LENGTH_BYTES = 8192;
...
class Headers {
  ...
  toAWSFormat() {
    ...
    for (const [key, values] of Object.entries(this.headers)) {
      ...
      const finalKey = key.includes('_') ? key : (0, _headerCase.default)(key);
 ...
 */
const rfc1123 = date => {
  const time = [leadingZero(date.getUTCHours()), leadingZero(date.getUTCMinutes()), leadingZero(date.getUTCSeconds())].join(':');
  return [`${dayNames[date.getUTCDay()]},`, leadingZero(date.getUTCDate()), monthAbbr[date.getUTCMonth()], date.getUTCFullYear(), time, 'GMT'].join(' ');
};
...
exports.cookieAsString = cookieAsString;
const IPV6 = 'IPV6';
const IPV4 = 'IPV4';
...
// hostname plus port
{
  ...
  // False means the hostname may have a subdomain
  hasPort: true
}];
...
const rewriteDomain = (domain, appHostname, targetHost) => {
 ...
 * @param setCookies {string[]} Array of set-cookie header values
 * @returns string[] of rewritten set-cookie header values
 */
exports.rewriteDomain = rewriteDomain;
const rewriteSetCookies = ({
  appHostname,
  setCookies,
  targetHost,
  logging
  ...
  const oldCookies = (0, _setCookieParser.parse)(setCookies, {
    decodeValues: false
  });
...
const rewriteProxyResponseHeaders = ({
  appHostname,
  caching,
  headers,
  headerFormat = 'http',
  ...
  const workingHeaders = new Headers(headers ? _objectSpread({}, headers) : {}, headerFormat);
  ...
  if (requestUrl) {
    ...
    logging && console.log(`Setting proxy response ${X_PROXY_REQUEST_URL} header to "${fullRequestUrl}"`);
  ...
  if (statusCode >= 301 && statusCode <= 308) {
    ...
    if (location) {
      ...
      const locUrl = new _url.URL(location, targetOrigin);
 ...
 * @type {string[]}
 */
exports.rewriteProxyResponseHeaders = rewriteProxyResponseHeaders;
const X_HEADERS_TO_REMOVE = exports.X_HEADERS_TO_REMOVE = ['x-api-key', 'x-mobify-access-key', 'x-apigateway-event', 'x-apigateway-context'];
 ...
 * @type {Object}
 */
const X_HEADERS_TO_ADD = exports.X_HEADERS_TO_ADD = {
  'x-mobify': 'true'
};
 ...
 * @private
 */
const ALLOWED_CACHING_PROXY_REQUEST_HEADERS = exports.ALLOWED_CACHING_PROXY_REQUEST_HEADERS = {
 ...
 */
const rewriteProxyRequestHeaders = ({
  caching = false,
  headers,
  headerFormat = 'http',
  targetProtocol,
  ...
  const workingHeaders = new Headers(_objectSpread({}, headers), headerFormat);
  ...
  if (originHeader && originHeader !== targetOrigin) {
    /* istanbul ignore else */
    if (logging) {
      console.log(`Rewriting proxy request Origin header from "${originHeader}" to "${targetOrigin}"`);
    }
...
exports.rewriteProxyRequestHeaders = rewriteProxyRequestHeaders;