...
const ALLOWED_CACHING_PROXY_REQUEST_METHODS = exports.ALLOWED_CACHING_PROXY_REQUEST_METHODS = ['HEAD', 'GET', 'OPTIONS'];
...
const configureProxy = ({
  ...
  const config = {
    ...
    onProxyRes: (proxyResponse, req) => {
      /* istanbul ignore next */
      if (!(0, _utils.isRemote)() && _utils.verboseProxyLogging) {
        console.log(`Proxy: ${proxyResponse.statusCode} response from ${proxyPath}${req.url}`);
      }
      ...
      // Rewrite key headers
      proxyResponse.headers = (0, _ssrProxying.rewriteProxyResponseHeaders)({
        appHostname,
        caching,
        targetHost,
        targetProtocol,
  ...
  return (req, res, next) => {
    ...
    if (!ALLOWED_CACHING_PROXY_REQUEST_METHODS.includes(req.method)) {
      return res.status(405).send(`Method ${req.method} not supported for caching proxy`).end();
    }
...
const configureProxyConfigs = (appHostname, appProtocol) => {
  (0, _utils.localDevLog)('');
  _ssrShared.proxyConfigs.forEach(config => {
    (0, _utils.localDevLog)(`Proxying ${config.proxyPath} and ${config.cachingPath} to ${config.protocol}://${config.host}`);
...
exports.configureProxyConfigs = configureProxyConfigs;