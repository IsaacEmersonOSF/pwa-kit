...
const RemoteServerFactory = exports.RemoteServerFactory = {
  ...
  _setupLogging(app) {
    app.use((0, _morgan.default)(function (tokens, req, res) {
      ...
      stream: _morganStream.morganStream
    }));
  },
  /**
  ...
  _setupSSRRequestProcessorMiddleware(app) {
    ...
    const ssrRequestProcessorMiddleware = (req, res, next) => {
      ...
      _ssrProxying.X_HEADERS_TO_REMOVE.forEach(key => {
  ...
  _setupHealthcheck(app) {
    app.get('/mobify/ping', (_, res) => res.set('cache-control', _constants.NO_CACHE).sendStatus(200).end());
  },
  /**
   * @private
   */
  ...
  _createHandler(app) {
    ...
    const handler = (event, context, callback) => {
      ...
      (err, response) => {
        ...
        .then(() => callback(err, (0, _ssrServer.processLambdaResponse)(response)))
  ...
  if (!options.allowCookies) {
    ...
    res.setHeader = function (header, value) {
      ...
      if (header && header.toLowerCase() !== _constants.SET_COOKIE && value) {
        ...
        console.warn(`Req ${res.locals.requestId}: ` + `Cookies cannot be set on responses sent from ` + `the SSR Server. Discarding "Set-Cookie: ${value}"`);
      }
    };
  }
